...
class TransformerEncoderLayer(nn.Layer):
    ...
    def __init__(self, d_model, n_heads, d_ffn, dropout=0.):
        ...
        self.self_mha_ln1 = nn.LayerNorm([d_model], epsilon=1e-6)
        self.self_mha_ln2 = nn.LayerNorm([d_model], epsilon=1e-6)

        self.ffn = PositionwiseFFN(d_model, d_ffn) # a 2 layer linear without act
        self.ffn_ln1 = nn.LayerNorm([d_model], epsilon=1e-6)
        self.ffn_ln2 = nn.LayerNorm([d_model], epsilon=1e-6)

        self.dropout = dropout

    def _forward_mha(self, x, mask, drop_n_heads):
        # Double LN: mha -> ln -> dropout -> residual -> ln
        context_vector, attn_weights = self.self_mha(x, x, x, mask, drop_n_heads)
        context_vector = self.self_mha_ln1(context_vector)
        context_vector = x + F.dropout(context_vector, self.dropout, training=self.training)
        context_vector = self.self_mha_ln2(context_vector)
        return context_vector, attn_weights

    def _forward_ffn(self, x):
        # Double LN: ffn -> ln -> act -> dropout -> residual -> ln
        out = self.ffn(x)
        out = self.ffn_ln1(out)
        out = F.relu(out)
        out = x + F.dropout(out, self.dropout, training=self.training)
        out = self.ffn_ln2(out)
        return out
...
class TransformerDecoderLayer(nn.Layer):
    ...
    def __init__(self, d_model, n_heads, d_ffn, dropout=0., d_encoder=None):
        ...
        self.cross_mha_ln = nn.LayerNorm([d_model], epsilon=1e-6)
    ...
    def _forward_cross_mha(self, q, k, v, mask, drop_n_heads):
        # Double LN: mha -> dropout -> residual -> ln
        context_vector, attn_weights = self.cross_mha(q, k, v, mask, drop_n_heads)
        context_vector = q + F.dropout(context_vector, self.dropout, training=self.training)
        context_vector = self.cross_mha_ln(context_vector)
        return context_vector, attn_weights
...
class MLPPreNet(nn.Layer):
    ...
    def forward(self, x, dropout):
        ...
        l1 = F.dropout(F.relu(self.lin1(x)), dropout, training=True)
        l2 = F.dropout(F.relu(self.lin2(l1)), dropout, training=True)
        return l2
...
class TransformerTTS(nn.Layer):
    ...
    def encode(self, text):
        ...
        encoder_padding_mask = masking.id_mask(text, 0, dtype=x.dtype)
...
class TransformerTTSLoss(nn.Layer):
    ...
    def forward(self, mel_output, mel_intermediate, mel_target, stop_logits, stop_probs):
        ...
        mel_loss1 = masked_l1_loss(mel_output, mel_target, mask1)
        mel_loss2 = masked_l1_loss(mel_intermediate, mel_target, mask1)
        ...
        stop_loss = masked_softmax_with_cross_entropy(stop_logits, stop_probs.unsqueeze(-1), mask2.unsqueeze(-1))
...
def weighted_mean(input, weight):
    ...
    NOTE: it is not a real weighted mean, it's the same as keras.
    Args:
